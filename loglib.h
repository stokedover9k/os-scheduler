/*
 * This logging library is taken from Dr. Dobbs Blog.
 *
...
#define TLOG \
  if (logINFO > FILELOG_MAX_LEVEL) ;          \
  else if (logINFO > FileLog::ReportingLevel() || !Output2FILE::Stream()) ; \
  else FileLog().Get(logINFO)
